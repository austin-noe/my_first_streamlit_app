...
y_data = [random.random() for values in x_axis]
...
- Changing size based on y
- tooltip shows x and y coordinate
- color is correlated to y 
- Changed column width
""")

scatter = alt.Chart(df).mark_point().encode(x = 'x', y = 'y', size= 'y', color = 'y' ,tooltip= ['x','y'])
...
source = pd.read_json('unemployment.json')
st.write(source)
...
- changed the colors of the negative and positive bars
- changed the width of the plot
"""
)
